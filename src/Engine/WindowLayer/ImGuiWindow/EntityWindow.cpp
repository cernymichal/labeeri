...
void modelSection(const Ref<ModelResource>& model) {
if (!model)
    return;
...
if (model->m_material) {
    if (castRef<ShadedMaterialResource>(model->m_material))
        materialSectionShaded(castRef<ShadedMaterialResource>(model->m_material));
    else if (castRef<FlatMaterialResource>(model->m_material))
        materialSectionFlat(castRef<FlatMaterialResource>(model->m_material));
    else
